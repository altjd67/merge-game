...
namespace MergeBoard.Editor
{
    ...
    public static class MergeBoardVerification
    {
        ...
        public static void VerifyRules()
        {
            ...
            var swapped = controller.Move(1, 2);
            Assert(swapped.Success && swapped.Swapped && controller.Board[1] == ItemStage.Seed && controller.Board[2] == ItemStage.Sprout, "다른 단계 교환");
            Assert(controller.Move(1, 3).Merged && controller.Move(2, 3).Merged && controller.Board[3] == ItemStage.Flower, "꽃 합성");
            ...
            var maximumLevelSwap = new MergeGameController(new BoardModel(flowerCells)).Move(0, 1);
            Assert(maximumLevelSwap.Success && maximumLevelSwap.Swapped, "최종 단계 아이템 교환");
            ...
            var generatorSwap = generator.Move(62, 54);
            Assert(generatorSwap.Success && generatorSwap.Swapped && generator.Board[62] == ItemStage.Seed && generator.Board[54] == ItemStage.SeedPack, "생성기와 아이템 교환");
        ...
        public static void VerifyDragInPlayMode()
        {
            ...
            Drag(game, 1, 2);
            Assert(game.Board[1] == ItemStage.Seed && game.Board[2] == ItemStage.Sprout, "합성 불가 아이템 교환");
            Drag(game, 1, -1);
            Assert(game.Board[1] == ItemStage.Seed && !game.BoardView.IsDragging, "보드 밖 복귀와 캡처 해제");
            Drag(game, 1, 3); Drag(game, 2, 3);
        ...
        public static void PrepareGeneratorUI()
        {
            ...
            var message = root.Find("안내").GetComponent<TextMeshProUGUI>();
            ...
            message.alignment = TextAlignmentOptions.Center; message.fontSize = 16; message.color = Color.white; message.text = "";
