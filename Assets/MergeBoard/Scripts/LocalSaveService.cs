...
namespace MergeBoard
{
    ...
    public sealed class LocalSaveService
    {
        ...
        private const string WebGlSaveKey = "MergeBoard.Progress";
#endif
        ...
        public GameState Load()
        {
            ...
            catch (Exception error) when (error is IOException || error is UnauthorizedAccessException || error is ArgumentException || error is PlayerPrefsException)
            {
        ...
        public bool Save(GameState state)
        {
            ...
            try
            {
                ...
                data.ToState();
#if UNITY_WEBGL && !UNITY_EDITOR
                PlayerPrefs.SetString(WebGlSaveKey, JsonUtility.ToJson(data));
                PlayerPrefs.Save();
#else
