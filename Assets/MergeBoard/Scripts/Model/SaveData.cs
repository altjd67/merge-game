    ...
    public sealed class SaveData
    {
        ...
        public static SaveData FromState(GameState state)
        {
            ...
            for (int index = 0; index < data.orderIds.Length; index++)
            {
                ...
                data.completedOrders[index] = state.Orders[index].Completed;
            }
            return data;
        }
        ...
        public GameState ToState()
        {
            ...
            {
                if (orderIds[index] != state.Orders[index].Id) throw new ArgumentException("주문 ID가 일치하지 않습니다.");
                state.Orders[index].Completed = completedOrders[index];
                if (completedOrders[index]) expectedCoins += state.Orders[index].Reward;
            }
            if (coins != expectedCoins) throw new ArgumentException("코인과 주문 완료 상태가 일치하지 않습니다.");
            state.Coins = coins;
            return state;
        }
    }
